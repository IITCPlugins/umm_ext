...
(() => {
    ...
    class RenderPath {
        constructor() {
            this.missionPaths = new window.L.FeatureGroup, window.addLayerGroup("UMM: Mission Paths", this.missionPaths, !0), 
            this.touchIcon = L.Browser.touch ? new L.DivIcon({
        ...
        drawEditMission(mission) {
            ...
            coordinatesList.forEach((ll, index) => {
                if (index > 0) {
                    ...
                    this.createDragMarker(half, index, mission.id, !0);
                }
            });
        ...
        movePortal(mission, portalID, target) {
            if (0 === portalID) {
                ...
                } else if (1 === mission.portals.length && (null == preMission ? void 0 : preMission.portals.includes(target.guid)) && confirm("Split mission ?")) {
    ...
    class RenderNumbers {
        constructor() {
            this.missionNumbers = new window.L.FeatureGroup, window.addLayerGroup("UMM: Mission Numbers", this.missionNumbers, !0);
        }
        isVisible() {
    ...
    const loadFile = async (state, inputFile) => {
        ...
        !0;
    }, title = "Ultimate Mission Maker", Button_button = (label, click, classes) => $("<button>", {
        text: label,
        click,
        ...
        html += "</div>", window.dialog({
            html,
            title: "Edit banner details - UMM v0.7.3",
            id: "umm-options",
            width: 400,
            buttons: [ dialogButton("< Main Menu", showUmmOptions), dialogButton("Save", () => succesfulSave(toggleMissionModeAfterSave)), dialogButtonClose() ]
    ...
    }, succesfulSave = toggleMissionModeAfterSave => {
        saveMissionSetDetails($("#umm-mission-set-name").val(), $("#umm-mission-set-description").val(), $("#umm-banner-length").val(), $("#umm-title-format").val()) && (updateCurrentActiveMissionSidebar(main.state), 
        notification(`${main.state.getBannerName()}\nMission details saved`), toggleMissionModeAfterSave && toggleMissionMode());
    }, updateMissionTitlePreview = () => {
        if ("" == $("#umm-mission-set-name").val() || "" == $("#umm-title-format").val() || isNaN(parseInt($("#umm-banner-length").val()))) $("#umm-mission-title-preview").text("Fill in all required fields"); else {
            const missionTitle = Missions.generateMissionTitle(1, parseInt($("#umm-banner-length").val()), $("#umm-mission-set-name").val(), $("#umm-title-format").val());
        ...
        let shouldStoreData = !0;
        missionSetName && "" != missionSetName ? (main.state.setBannerName(missionSetName), 
        $("#umm-mission-set-name-error").css("display", "none")) : (shouldStoreData = !1, 
        $("#umm-mission-set-name-error").css("display", "block")), missionSetDescription && "" != missionSetDescription ? (main.state.setBannerDesc(missionSetDescription), 
        $("#umm-mission-set-description-error").css("display", "none")) : (shouldStoreData = !1, 
        $("#umm-mission-set-description-error").css("display", "block"));
        const plannedLength = parseInt(null != plannedBannerLength ? plannedBannerLength : "");
        return plannedLength && !isNaN(plannedLength) ? (main.state.setPlannedLength(plannedLength), 
        $("#umm-mission-planned-banner-length-error").css("display", "none")) : (shouldStoreData = !1, 
        $("#umm-mission-planned-banner-length-error").css("display", "block")), titleFormat && "" != titleFormat ? (main.state.setTitleFormat(titleFormat), 
        $("#umm-mission-title-format-error").css("display", "none")) : (shouldStoreData = !1, 
        $("#umm-mission-title-format-error").css("display", "block")), !!shouldStoreData && (main.state.save(), 
        !0);
    }, editActiveMission = () => {
        const html = $("<div>", {
            class: "umm-mission-picker-btn"
        }).append("Select a mission number:<br>", $("<select>", {
            id: "umm-mission-picker",
            class: "umm-mission-picker",
            change: updateMissionInfo
        }).css({
            "margit-right": "1em"
        }), Button_button("Select", onMissionSelect), Button_button("Zoom to mission", onZoomToMission), $("<div>", {
            id: "um-mission-pikcer-info"
        }), Button_button("Split", onMissionSplit), Button_button("Reverse", onMissionReverse), $("<br>"), Button_button("Merge with previous", onMergePrevious), Button_button("Merge next into this", onMergePost));
        window.dialog({
            html,
            title: `${title} v0.7.3`,
            id: "umm-options",
        ...
        const mission = selectedMission();
        mission && !main.state.isCurrent(mission.id) ? (setCurrentMission(mission.id), main.missionModeActive ? (renderPortalDetails(null), 
        startEdit()) : (mission.show(), notification(`Current working mission set to #${mission.id + 1}`)), 
        $("#dialog-umm-options").dialog("close")) : notification("Active mission not changed.");
    }, onZoomToMission = () => {
        const mission = selectedMission();
        mission ? mission.show() : notification("Can't zoom in on this mission. No portals.");
    }, onMissionSplit = () => {
        var _a, _b;
        const missions = main.state.missions, mission = selectedMission();
        if (!mission) return;
        ...
        mustMerge && !confirm("Mission(s) already contain portals.\nMission(s) will be merge first?") || (missions.splitIntoMultiple(mission, count), 
        main.state.save(), updateMissionInfo(), updateMissionList(), main.redrawAll());
    }, onMergePrevious = () => {
        const missions = main.state.missions, mission = selectedMission();
        if (!mission) return;
        ...
        main.state.missions.merge(previous, mission), main.state.save(), updateMissionInfo(), 
        updateMissionList(), main.redrawAll();
    }, onMergePost = () => {
        const missions = main.state.missions, mission = selectedMission();
        if (!mission) return;
        let next = missions.next(mission);
        for (;0 === (null == next ? void 0 : next.portals.length); ) next = missions.next(next);
        next && (main.state.missions.merge(mission, next), main.state.save(), updateMissionInfo(), 
        updateMissionList(), main.redrawAll());
    }, onMissionReverse = () => {
        const mission = selectedMission();
        mission && (mission.portals.reverse(), main.state.save(), updateMissionInfo(), main.redrawAll());
    }, showUmmOptions = () => {
        var _a, _b, _c;
        const state = main.state, html = $("<div>", {
            ...
            text: "Banner data"
        }), $("<br>"), "Banner name: <b><span>" + (null !== (_a = state.getBannerName()) && void 0 !== _a ? _a : "N/A") + "</span></b><br>", "Banner description: <b><span>" + (null !== (_b = state.getBannerDesc()) && void 0 !== _b ? _b : "N/A") + "</span></b><br>", "Mission title format: <b><span>" + (null !== (_c = state.getTitleFormat()) && void 0 !== _c ? _c : "N/A") + '</span></b> <span title="Title format allows:&#10;N = Mission number without leading 0 (if required by banner length)&#10;NN = Mission number with leading 0&#10;M = Planned banner length&#10;T = (mission title)&#10; &#10;eg. T N-M or NN.M T">(?)</span><br>', "Planned banner length: <b><span>" + state.getPlannedLength().toString() + "</span></b><br>", "Length: <b><span>" + window.formatDistance(state.missions.getTotalDistance()) + "</span></b><br>", validateMissions(state)), $("<p>").append($("<b>", {
            text: "Mission options"
        }), $("<br>"), "Layers: ", '<label style="user-select: none"><input type="checkbox" onclick="window.plugin.umm.toggleLayerPaths(this.checked)" id="umm-layercheckbox-paths"' + (main.renderPath.isVisible() ? " checked" : "") + ">Mission Paths</label>", '<label style="user-select: none"><input type="checkbox" onclick="window.plugin.umm.toggleLayerNumbers(this.checked)" id="umm-layercheckbox-numbers"' + (main.renderNumbers.isVisible() ? " checked" : "") + ">Mission Numbers</label>", Button_button("Edit banner details", editMissionSetDetails, "w-full"), Button_button("Change active mission #", editActiveMission, "w-full"), Button_button("Zoom to view all missions", () => state.missions.zoom(), "w-full")), $("<hr>"), Button_button("Split mission", splitMissionOptions, "w-full"), Button_button("Merge missions", mergeMissions, "w-full"), Button_button("Reverse mission", reverseMission, "w-full"), $("<hr>"), Button_button("Clear ALL missions data", confirmClear, "w-full"), $("<b>", {
            text: "Import/Export"
        ...
        confirm("Are you sure you want to clear ALL missions data?") && clearMissionData();
    }, confirmLoad = async event => {
        confirm("Are you sure you want to overwrite the current mission data?") && (await (async (event, state) => {
        ...
        })(event, main.state), main.state.checkAllPortals(), main.redrawAll(), updateCurrentActiveMissionSidebar(main.state), 
        updatePortalCountSidebar(), main.state.missions.zoom());
        ...
        height: 16,
        "margin-top": "7px"
    })), updateCurrentActiveMissionSidebar = state => {
        ...
        mission.hasPortals() ? showMission(mission) : main.missionModeActive && notification(`${state.getBannerName()}\nStart of mission #${state.getCurrent() + 1}\nSelect start portal.`);
    }, previousMission = () => {
        const state = main.state;
        ...
        mission.hasPortals() && (mission.show(), updatePortalCountSidebar(), main.missionModeActive ? notification(`Mission mode active.\n${main.state.getBannerName()}\nCurrent mission #${main.state.getCurrent() + 1}\nSelect next portal`) : notification(`${main.state.getBannerName()}\nCurrent active mission set to #${main.state.getCurrent() + 1}`));
    };
    let lastPortal, missionModeResuming = !1;
    const clearMissionData = () => {
        main.state.reset(), main.state.save(), updateCurrentActiveMissionSidebar(main.state), 
        main.missionModeActive && toggleMissionMode(), main.redrawAll();
    }, removeLastPortal = () => {
        var _a;
        if (!main.missionModeActive) return void notification("Only valid in edit mode");
        const mission = main.state.getEditMission();
        mission && mission.portals.length > 0 ? (mission.portals.remove(mission.portals.length - 1), 
        main.state.save(), main.redrawAll(), mission.focusLastPortal() || (renderPortalDetails(null), 
        ...
        (null == editMission ? void 0 : editMission.hasPortals()) ? (missionModeResuming = !0, 
        editMission.show(), window.renderPortalDetails(editMission.portals.get(-1).guid), 
        ...
        var _a, _b;
        if (main.state.getPlannedLength() > 1) return confirm("It's currently only supported to split 1 single mission, do you want to merge all missions now and continue?\r\n\r\nThis can't be undone.") ? (main.state.missions.mergeAll(), 
        void splitMissionStart(remainderAtEnd)) : void 0;
        ...
        if (!mission) return;
        const numPortals = null == mission ? void 0 : mission.portals.length, numRestPortals = numPortals % numMissions;
        let textMessage = `Your path of ${numPortals} will be divided into ${numMissions} missions of ${Math.floor(numPortals / numMissions)} portals each.`;
        numRestPortals > 0 && (textMessage += remainderAtEnd ? ` The remaining ${numRestPortals} portal(s) will be added to the last mission.` : ` The remaining ${numRestPortals} portal(s) will be equaly divided between the first missions.`), 
        textMessage += "\r\n\r\nThis process can be reversed using the merge missions feature. Do you want to continue?", 
        confirm(textMessage) && (main.state.missions.splitIntoMultiple(mission, numMissions, remainderAtEnd), 
        main.state.save(), main.redrawAll(), updatePortalCountSidebar());
    }, mergeMissions = () => {
        confirm("Are you sure you want to merge all your missions into 1?\r\n\r\nThis can't be undone.") && (main.state.missions.mergeAll(), 
        main.state.setCurrent(0), main.redrawAll());
    }, reverseMission = () => {
        ...
        mission ? (mission.reverse(), state.save(), main.redrawAll()) : alert(`This mission doesn't exist, enter a value between 1-${state.getPlannedLength()}.`);
    }, setCurrentMission = missionId => {
        main.state.setCurrent(missionId), main.state.save(), updateCurrentActiveMissionSidebar(main.state), 
        main.redrawAll();
        ...
        portal.objective.passphrase_params = passphrase_params, main.state.save(), notification("Question and passphrase saved");
    }, updateActionSelect = () => {
        const portal = currentPortal();
    ...
    const main = new class UMM_Ext {
        ...
        init() {
            __webpack_require__(768), this.state = new State, $("body").append("<div class='umm-notification' style='display:none'></div>"), 
            (() => {
            ...
            window.addHook("portalSelected", event => (data => {
                const state = main.state;
                if (!main.missionModeActive || missionModeResuming) return void (missionModeResuming = !1);
                ...
                if (mission.portals.includes(portalToAdd.guid)) {
                    ...
                    if (0 === mission.portals.length && preMission && preMission.portals.includes(portalToAdd.guid) && !preMission.portals.isStart(portalToAdd) && !preMission.portals.isEnd(portalToAdd) && confirm("Split mission?")) {
                    ...
                    mission.portals.add(portalToAdd), state.save(), main.redrawAll(), notification(`${main.state.getBannerName()}\nAdded to mission #${main.state.getCurrent() + 1}`);
                }
            ...
            this.missionModeActive = !1, this.redrawAll(), updateCurrentActiveMissionSidebar(this.state);
        }
        redrawAll() {
            this.renderPath.redraw(), this.renderNumbers.redraw(), updatePortalCountSidebar();
        }
    };
